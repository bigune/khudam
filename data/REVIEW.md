...
**Correction applied** via [`scripts/fix-yi-digraph.ts`](../scripts/fix-yi-digraph.ts): medial ᠶᠢ → ᠢ (drop the spurious ᠶ) in the **2,780** entries whose Cyrillic key contains й. Шийдвэрийн бүрэн үндэслэл: [ENCODING.md, Шийдвэр 001](ENCODING.md). / Full rationale and citations: [ENCODING.md, Decision 001](ENCODING.md). These remain `verified: false` — a machine correction is not human verification, so they still need review before anyone sets `verified: true`.
