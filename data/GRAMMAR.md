...
- **Nadmid 1990** — Я. Надмид, *Монгол бичгийн зөв бичих толь бичиг*, Улсын хэвлэлийн газар, Улаанбаатар, 1990. Two PDFs circulate. The coo.mn scan (<https://coo.mn/uploads/a/Almas/dusal2020/mongol-bichig-durem.pdf>) is 45 pages of page images with no text layer at all. A **digitally typeset edition also exists** (Acrobat Distiller 2009, from `Durem.doc`) whose Cyrillic rule text extracts cleanly with `pdftotext` — that one is searchable, and is what makes quoting Nadmid practical. Its монгол бичиг is a legacy Private-Use-Area font, so **script code points remain unusable from either edition** and must be typed by a human. Copyrighted work: rules and facts are restated here with citation; the text is not copied and the PDF is not committed to this repo. Its traditional-script examples are page images — any Unicode re-entry must be typed and human-verified, never scraped.
