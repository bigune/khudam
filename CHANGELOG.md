...
- **The expert review pipeline** — the piece that lets this project verify anything at scale, and a fix for the weakest link in the old one. Verification used to end at the maintainer reading a weekly JSON diff, which is the wrong place for it twice over: traditional script cannot be judged in a diff (several distinct letters share identical glyphs, and a diff renders nothing at all), and the maintainer does not read монгол бичиг well enough to be the last word anyway. Judgement now belongs to a trusted reviewer on a page built for it, and the maintainer's job is merging something already judged.
  - **[The review page](https://khudam.suray.mn/review)** shows every spelling as script **and** as its code points. That is the feature, not a detail: shown only the rendered form, a reviewer can confirm a wrong encoding in perfect good faith and nothing downstream would catch it — the validator checks that code points are inside the Mongolian block, not that they are the intended ones. Names and hex only, never isolated glyphs, because a Mongolian letter alone renders in a different form from the one it takes inside a word. The invisible ones — the free variation selectors, MVS, the NNBSP that makes a written-apart suffix a suffix — are marked, since a difference nobody can see is exactly where a wrong encoding hides.
  - It reads a bundle compiled from `data/` at deploy time (`bun run build:review-bundle`), **never the mailbox**. The mailbox never holds all the votes — the weekly job drains and deletes, and the tallies live in `data/stats/reports.json` — so a live query would show one week and call it the whole picture. It would also need read access to Supabase, where today nothing in any browser can read a single row. The cost is latency, and at this project's cadence that is the right trade.
  - Send inserts rows into a new `decisions` table and can do nothing else. A workflow run drains it, hash-checks every stamp against `data/reviewers.json` **in git**, and transcribes what survives into one pull request. The page proposes; git remains the database — which is what lets the page be a static file with no credential worth stealing.
  - **A rejection is never applied.** It is written into `data/REVIEW.md` with the file and the line to delete. Removing a candidate is the one edit that loses data and cannot be undone by merging something else, so it is the one thing no grant can cause: a leaked link can verify a spelling, and cannot empty the lexicon.
- **The converter can be told a spelling is right** — a ✓ beside the existing ⚑. Until now the only thing a reader could say about a candidate was that something was wrong with it, which is the rarer half of what somebody who reads монгол бичиг knows; the commonest true statement available had no button. For a stranger the vote orders the review page and shows as support beside the candidate. For a browser holding a reviewer grant it is an attestation, through the machinery queue answers already use.
...
- Two suffix rows the table had been missing, both cited: **privative -гүй → ᠦᠭᠡᠢ** (`GRAMMAR.md` G14) and the **substantive genitive -х → ᠬᠢ** (G15). номгүй → `ᠨᠣᠮ ᠦᠭᠡᠢ`, номынх → `ᠨᠣᠮ ᠤᠨ ᠬᠢ`, багшийнх → `ᠪᠠᠭᠰᠢ ᠶᠢᠨ ᠬᠢ`. The privative was the single largest hole left in the engine — 1,114 forms in the measurement set, 0.2% of them resolved; it now reads 52.2%, and overall coverage goes from 45.4% to **51.8%** at 94.1% precision.
  ...
  **The privative is written apart after every stem**, including vowel-final ones: усгүй → `ᠤᠰᠤ ᠦᠭᠡᠢ`, not a joined form. Thirty seed entries write it joined and contracted (аальгүй → `ᠠᠭᠠᠯᠢᠭᠦᠢ`), which looked like a vowel/consonant alternation of the kind governing the genitive and instrumental, and shipped briefly as one. Forms from mongoltoli.mn settled it otherwise: ус is vowel-final and still apart. The split is lexical rather than phonological — the joined words are lexicalized adjectives (аальгүй "improper", хичээнгүй "diligent", which even assimilates ᠨ → ᠩ before the ᠭ), and by ground rule 1 those are lexicon entries, not rule output. An exact match already keeps the rule out of their way.

  Both rows stay `verified: false`, with what is open recorded in `data/REVIEW.md`: whether ᠬᠢ has a harmony pair, and two seed entries mongoltoli.mn contradicts outright (төсөргүй stored joined and with a different stem vowel, ааггүй stored joined). A citation is not verification; a human reading монгол бичиг still is.

- **Open encoding question, nothing changed pending a ruling** (`data/REVIEW.md`): every form supplied from mongoltoli.mn ends the privative in **ᠶ U+1836** — `ᠨᠣᠮ ᠦᠭᠡᠶ`, `ᠤᠰᠤ ᠦᠭᠡᠶ`, `ᠲᠥᠰᠦᠷ ᠦᠭᠡᠶ`, `ᠬᠢᠴᠢᠶᠡᠩᠭᠦᠶ` — where every form of ours ends in **ᠢ U+1822**. Four for four is a convention, not a slip.

  The reach is well beyond one suffix: **1,372 lexicon candidates** end in vowel + ᠢ (аатай, аанай, …) against exactly one in vowel + ᠶ, and eight suffix rows are affected including the comitative ᠲᠠᠢ/ᠲᠡᠢ that ENCODING.md Decision 002 pins by code-point test. A ruling would revise Decisions 001–002 rather than sit beside them. It is also narrower than Decision 001, which removed the medial *digraph* ᠶᠢ; this is about which single letter spells a word-final postvocalic i, a position that decision never examined. Deciding it needs UTN #57's final-form table read at code-point level, which is a human's job.
...
- **Revocation now reaches everything a grant ever earned.** A self-review of the expert pipeline found the gap: labels were resolved from grants once, at fold time, and then lived in `data/stats/reports.json` across weeks — so revoking a grant stopped its *new* stamps but left its recorded attestations staging verifications from the file, and deleting the highest roster line would have let `reviewer:add` hand that label to a different person. Revocation is now a tombstone (`bun run reviewer:revoke rN` — the line stays, so a label is never reissued), every run prunes recorded labels against the roster before reading them, and every fast-track flip is on the record in `stats/reports.json`: when all the grants behind one are revoked, the next run stages `verified: false` in the same kind of diff that staged the opposite. Hand-written verifications have no record and are never touched.
- **A trusted reviewer's "no" to a proposed spelling used to vanish.** It became a tally about a candidate that exists in no shard, which the same run discarded — unrendered, unpersisted — while the proposal resurfaced on the review page every deploy. It is now a decline: the proposal's reports close, the decline is written up with its labels, and (because a closed report cannot become a mechanical addition) two anonymous sessions can no longer add a spelling in the same run an expert said it is wrong.
- **An acceptance the schema refused survives past one week.** It used to live exactly one regeneration of REVIEW.md; it is now carried in `stats/reports.json` and re-tried each run — label the stored candidate by hand and the next run writes the acceptance, attestation included. The review page also stops re-asking about a proposal that is accepted and merely waiting on a `sense`, and the page now sends the visibly prefilled meaning label instead of holding the acceptance back as incomplete.
- **The weekly export leaves a one-minute grace margin.** `created_at` is transaction start time, so an insert committing mid-export could sort *below* the already-exported rows and be skipped as processed next week — then deleted. Rows younger than the margin are now simply next week's mail.

### Changed

- **One trusted reviewer now stages a verification, where two were needed before.** `ATTESTATION_THRESHOLD` is 1, deliberately and in the open, because one trusted expert is who this project realistically has — and a threshold nobody can reach verifies nothing. A lexicon that stays 100% unverified because the rule was admirable is not a better outcome than one verified by the person who actually reads the script. What keeps it honest rather than sloppy: every flip records its reviewer's opaque label, so the single-reviewer era stays auditable and revoking a grant still mechanically un-verifies everything it ever attested; the veto stays live, so any later trusted "no" reopens the entry; the maintainer still merges, and the per-pull-request cap still keeps the fast-track section short enough to read; and no grant can delete anything. Raise it back to 2 when there are two reviewers — it is one constant plus the documents that quote it. Entries verified under the old rule are **not** retroactively un-verified: they were valid under the rule that applied, they are auditable by label, and a dispute reopens them one at a time.
